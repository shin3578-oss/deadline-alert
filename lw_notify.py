# ==========================================================
# ※これは公開リポジトリ側の複製です（2026-08-31）。
#   apotool-automation（非公開）にも同じものがありますが、非公開リポジトリの実行は
#   GitHub Actions の無料枠（月2,000分）を消費し、しかもジョブ単位で1分に切り上げられます。
#   数秒の通知でそれを払うのが無駄なので、個人DMの通知はこちら（公開＝無料無制限）へ寄せています。
#   チャンネル投稿はチャンネルIDのSecretを持つ apotool 側を使ってください。
